...
class BasicBattery(BaseBattery):
    ...
    def __init__(self, state_of_charge):
        ...
        self.calculate_discharge_capacity_from_energy = lambda x: 136.92 - math.sqrt(18747.06027 - 2.32857*x)
    ...
    def update_array(self, cumulative_energy_array):
        ...
        stored_energy_array = np.full(cumulative_energy_array.size, fill_value=self.stored_energy)
        ...
        energy_discharged_array = np.full(cumulative_energy_array.size, fill_value=self.max_energy_capacity) - \
                                  stored_energy_array

        discharge_capacity_array = np.array([self.calculate_discharge_capacity_from_energy(energy)
                                             for energy in energy_discharged_array])

        soc_array = self.calculate_soc_from_discharge_capacity(discharge_capacity_array)

        return stored_energy_array, discharge_capacity_array, soc_array
