...
class Libraries:
    def __init__(self, raiseExceptionOnFail=True):
        """"
        :param raiseExceptionOnFail: Boolean to control whether an exception should be raised
        if go binaries cant be found. Should not be set to false unless that scenario is handled.
        """
    ...
    def GetGoDirectory(self):
        ...
        # Try to find current operating system
        try:
            ctypes.cdll.LoadLibrary(f"{go_directory}/binaries/MacM1/compatibility_check.so")
            return f"{go_directory}/binaries/MacM1"
        except:
            pass
        try:
            ctypes.cdll.LoadLibrary(f"{go_directory}/binaries/Linux/compatibility_check.so")
            return f"{go_directory}/binaries/Linux"
        except:
            pass
        try:
            ctypes.cdll.LoadLibrary(f"{go_directory}/binaries/Windows/compatibility_check.so")
            return f"{go_directory}/binaries/Windows"
        except:
