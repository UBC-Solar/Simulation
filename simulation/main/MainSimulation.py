...
class Simulation:
    ...
    def run_model(self, speed=np.array([20, 20, 20, 20, 20, 20, 20, 20]), plot_results=True, verbose=False, **kwargs):
        ...
        if plot_results:
            ...
            if self.race_type == "FSGP":
                ...
                helpers.route_visualization(self.gis.path, self.waypoints, visible=verbose)
    ...
    def __run_simulation_calculations(self, speed_kmh, verbose=False):
        ...
        if (self.race_type == "ASC"):
            speed_kmh = helpers.step5_match_coords(self.gis.path, self.gis.path_distances, np.divide(speed_kmh, 3.6), self.waypoints, verbose=False)[:self.simulation_duration+1]
