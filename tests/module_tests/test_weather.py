...
def weather():
    ...
    weather_calculations = OpenWeatherForecast(
        coords=route_coords, race_type="ASC", provider="OPENWEATHER"
    )
...
@pytest.mark.skip(
    reason="This test cannot be fixed for how Go is currently being used."
)
def test_compare_golang_python_get_weather_in_time(weather):
    ...
    go_gwit_result = weather.golang_calculate_closest_timestamp_indices(
        unix_timestamps, dt_local_array
    )

    python_gwit_result = weather._python_calculate_closest_timestamp_indices(
        unix_timestamps, dt_local_array
    )
